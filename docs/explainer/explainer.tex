...
The shortest route in that room scores 0.5457, so the extra distance genuinely buys clarity. And the two ends sit 0.0065 apart, which means any target outside that narrow band is now settled: above it, provably unreachable; at or below the achieved value, definitely reached.
...
Eight rooms, four distance budgets each, so 32 combinations in total. \textbf{Every one holds, with no violations.} The intervals range from 0.0065 apart at the tightest to 0.0567 at the widest.
...
What survives even that is a single constant in the obstacle argument. The proof guarantees a detour of no more than about 9.28 cell-widths near a corner; measuring actual point pairs beside actual corners finds the worst real detour is 2.53. So the constant is loose by a factor of about 3.7.

\begin{quotebox}
That is not an error, because a proof must hold in the worst case and the worst case is rarely met. The repository names it as the single highest-leverage improvement outstanding: a sharper argument for those cells is where the next gain lives.
\end{quotebox}

Publishing the location of your own weakest step, with a measurement of how weak it is, is unusual. It is also the reason the rest of the numbers can be taken at face value.
