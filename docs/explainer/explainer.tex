...
% reader. The figures are the ones in docs/img/, produced by the
% repository's own tools.
%
% Build from this directory:  latexmk -pdf explainer.tex
%
% Every passage that quotes a number sits between a pair of markers reading
% "generated:<block>" and "/generated:<block>", each on its own comment line,
% and is written by tools/build_explainer.py from the records in results/.
% Do not edit inside a marker; the next run overwrites it. Prose outside the
% markers is written by hand and is never touched.
%
% This used to say that the numbers here had to be re-checked against the
% README by hand, because nothing enforced them. Nothing did, and they went
% stale: this file was still quoting a narrowest gap of 0.0065 after the
% results had moved to 0.0064, and since the site's drift checker asks
% whether a number is still findable in this repository rather than whether
% it is still true, that one stale sentence held a stale claim on the site
% green. Run tools/build_explainer.py --check to fail instead of drifting.
